...
%MYDIRBLOCK%..\..\tools\cygwin_bin\procServ.exe --logstamp --logfile="%IOCCYGLOGROOT%/BLOCKSVR-%%Y%%m%%d.log" --timefmt="%%c" --restrict --ignore="^D^C" --name=BLOCKSVR --pidfile="/cygdrive/c/windows/temp/EPICS_BLOCKSVR.pid" %BLOCKSERVER_CONSOLEPORT% %BLOCKSERVER_CMD% 
