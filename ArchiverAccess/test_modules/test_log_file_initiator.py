...
class TestLogFileInitiator(unittest.TestCase):
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_from_1_to_0_THEN_log_file_created(self):
        ...
        archive_data_source = self._set_up_data_source(initial_pv_value=1, final_pv_value=0, log_period_in_second=log_period_in_second)
        log_file_initiator = self._create_log_file_intiator(archive_data_source)

        log_file_initiator.check_write()

        self.log_file_creator.write.assert_called_once()
        logging_time_period = self.log_file_creator.write.call_args[0][0]
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_0_to_X_THEN_log_file_not_created(self):
        archive_data_source = self._set_up_data_source(initial_pv_value=0, final_pv_value=1)
        log_file_initiator = self._create_log_file_intiator(archive_data_source)

        log_file_initiator.check_write()

        self.log_file_creator.write.assert_not_called()

    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_0_to_0_THEN_log_file_not_created(self):
        archive_data_source = self._set_up_data_source(initial_pv_value=0, final_pv_value=0)
        log_file_initiator = self._create_log_file_intiator(archive_data_source)
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_1_to_disconnect_THEN_log_file_created(self):
        archive_data_source = self._set_up_data_source(initial_pv_value=1, final_pv_value="disconnect")
        log_file_initiator = self._create_log_file_intiator(archive_data_source)

        log_file_initiator.check_write()

        self.log_file_creator.write.assert_called()

    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_disconnected_to_0_THEN_log_file_not_created(self):
        archive_data_source = self._set_up_data_source(initial_pv_value="disconnect", final_pv_value=1)
        log_file_initiator = self._create_log_file_intiator(archive_data_source)
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_twice_from_1_to_0_THEN_log_file_created(self):
        ...
        archive_data_source = self._set_up_data_source(initial_pv_value=1, log_period_in_second=log_period_in_second, data_changes=data_changes)
        ...
        arg_list = self.log_file_creator.write.call_args_list
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_twice_from_1_to_0_over_two_different_write_checks_THEN_two_log_files_created(self):
        ...
        data_changes = [[],
                        ...
                        (datetime(2017, 1, 1, 1, 2, 2), 0, 1),
                        (datetime(2017, 1, 1, 1, 3, 2), 0, 0)]]
        archive_data_source = self._set_up_data_source(initial_pv_value=1, log_period_in_second=log_period_in_second, data_changes=data_changes, sample_ids=sample_ids)
        expected_period = timedelta(seconds=log_period_in_second)
        expected_logging_stop_time1 = datetime(2017, 1, 1, 1, 1, 2)
        expected_logging_start_time2 = datetime(2017, 1, 1, 1, 2, 2)
        expected_logging_stop_time2 = datetime(2017, 1, 1, 1, 3, 2)
    ...
    def _set_up_data_source(self,
        ...
        initial_archiver_data_value.sample_time = expected_logging_start
        initial_archiver_data_values = [initial_archiver_data_value]
        ...
        self.config = ConfigBuilder("log_file{start_time}").trigger_pv("my_pv").logging_period(log_period_in_second).build()
        return archive_data_source

    def _create_log_file_intiator(self, archive_data_source):
        self.log_file_creator = Mock()
        self.log_file_creator.write = Mock()
        return LogFileInitiatorOnPVChange(self.config, archive_data_source, self.log_file_creator)
