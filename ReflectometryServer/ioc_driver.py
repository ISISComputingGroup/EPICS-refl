...
CorrectedReadbackUpdate = namedtuple(
...
class IocDriver:
    ...
    def __init__(
        ...
        out_of_beam_positions: Optional[List[OutOfBeamPosition] | float | int] = None,
        ...
        if out_of_beam_positions is None:
            ...
            try:
                ...
                self.component.beam_path_rbv.axis[
                    component_axis
                ].park_sequence_count = self._out_of_beam_lookup.get_max_sequence_count()
                self.component.beam_path_set_point.axis[
                    component_axis
                ].park_sequence_count = self._out_of_beam_lookup.get_max_sequence_count()
                self.component.beam_path_set_point.axis[component_axis].add_listener(
    ...
    def _on_define_value_as(self, new_event):
        ...
        logger.info(
            "Defining position for axis {name} to {corrected_value} (uncorrected {new_value}). "
            "From sp {sp} and rbv {rbv}".format(
                name=self._motor_axis.name,
                corrected_value=correct_position,
                new_value=new_event.new_position,
                sp=self._sp_cache,
                rbv=self._rbv_cache,
            )
        )
    ...
    def _on_correction_update(self, new_correction_value):
        ...
        """
        description = "{} on {} for {}".format(
            new_correction_value.description, self.name, self.component.name
        )
    ...
    def initialise_setpoint(self):
        ...
        if autosaved_value is None:
            corrected_axis_setpoint = self._engineering_correction.init_from_axis(
                self._motor_axis.sp
            )
        else:
            corrected_axis_setpoint = self._engineering_correction.from_axis(
                self._motor_axis.sp, autosaved_value
            )

        if self._out_of_beam_lookup is not None:
            beam_interception = beam_path_setpoint.calculate_beam_interception()
            distance_relative_to_beam = corrected_axis_setpoint - beam_path_setpoint.axis[
                self.component_axis
            ].get_displacement_for(0)

            max_sequence_count = self._out_of_beam_lookup.get_max_sequence_count()
            in_beam_status, is_at_sequence_end = self._get_in_beam_status(
                beam_interception,
                self._motor_axis.sp,
        ...
        beam_path_setpoint.axis[self.component_axis].init_displacement_from_motor(
            corrected_axis_setpoint
        )
    ...
    def _axis_will_move(self):
        ...
        """
        return (
            not self.at_target_setpoint()
            and self.component.beam_path_set_point.axis[self.component_axis].is_changed
        )
    ...
    def _backlash_duration(self):
        ...
        """
        backlash_distance, distance_to_move, is_within_backlash_distance = (
            self._get_movement_distances()
        )
    ...
    def _get_movement_distances(self):
        ...
        is_within_backlash_distance = (
            min([0.0, backlash_distance]) <= distance_to_move <= max([0.0, backlash_distance])
        )
    ...
    def get_max_move_duration(self):
        ...
        if self._axis_will_move() and self._synchronised and not is_to_from_park:
            ...
            logger.debug(
                "Shortest move duration for {}: {:.2f}s ({:.2f}s base; {:.2f}s backlash)".format(
                    self.name, duration, base_move_duration, backlash_duration
                )
            )
        return duration
    ...
    def perform_move(self, move_duration, force=False):
        ...
        if component_sp is not None and (self._axis_will_move() or force):
            ...
            if move_duration > 1e-6 and self._synchronised and not is_to_from_park:
                ...
                self._motor_axis.velocity = max(
                    self._motor_axis.min_velocity, self._get_distance() / move_duration
                )
            else:
    ...
    def rbv_cache(self):
        ...
        if self._rbv_cache is None:
            raise ValueError(
                "Axis {} not initialised. Check configuration is correct and motor IOC is running.".format(
    ...
    def _get_component_sp_and_is_to_from_parking(
        ...
        """
        if (
            not self.component.beam_path_set_point.axis[self.component_axis].is_in_beam
            ...
            STATUS_MANAGER.update_active_problems(
                ProblemInfo(
                    "No out of beam position defined for motor_axis",
                    self.name,
    ...
    def _on_update_rbv(self, update):
        ...
        """
        corrected_new_value = self._engineering_correction.from_axis(
    ...
    def _propagate_rbv_change(self, update):
        ...
        if self._out_of_beam_lookup is not None:
            ...
            distance_relative_to_beam = self.component.beam_path_rbv.axis[
                self.component_axis
    ...
    def _get_in_beam_status(self, beam_intersect, value, distance_relative_to_beam, parking_index):
        if self._out_of_beam_lookup is not None:
            in_beam_status, is_at_sequence_position = self._out_of_beam_lookup.out_of_beam_status(
                beam_intersect, value, distance_relative_to_beam, parking_index
            )
        else:
    ...
    def _on_update_sp(self, update):
        ...
        """
        self._sp_cache = self._engineering_correction.from_axis(
            update.value, self._get_component_sp(True)
        )
