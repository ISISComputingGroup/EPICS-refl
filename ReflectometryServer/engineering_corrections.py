    ...
    def correction(self, setpoint: float) -> float:
        ...
        try:
            return self._user_correction_function(
                setpoint, *[param.sp_rbv for param in self._beamline_parameters]
            )
            ...
            if setpoint is None or None in [param.sp_rbv for param in self._beamline_parameters]:
                ...
                STATUS_MANAGER.update_error_log(
                    "Engineering correction, '{}', raised exception '{}' ".format(
                        self.description, ex
                    ),
                    ex,
                )
                STATUS_MANAGER.update_active_problems(
                    ProblemInfo(
                        "Engineering correction throws exception",
                        self.description,
                        Severity.MINOR_ALARM,
                    )
                )
...
class GridDataFileReader:
    ...
    def read(self) -> None:
        ...
        with self._open_file(self._filename) as correction_file:
            ...
            try:
                ...
                raise IOError(
                    "Problem with data in '{}', line {} error {}".format(
                        self._filename, reader.line_num, e
                    )
                )
            except StopIteration:
                raise IOError(
                    "No data found in file for the grid data engineering correction '{}'".format(
                        self._filename
                    )
                )
    ...
    def _read_data(self, reader: csv.DictReader) -> None:
        ...
        for row in reader:
            if len(row) != len(self.variables) + 1:
                raise IOError(
                    "Line {} should have the same number of entries as the header in "
    ...
    def _open_file(filename: str) -> Generator[TextIO, None, None]:
        ...
        if not os.path.isfile(full_path):
            raise IOError(
                "No such file for interpolation 1D engineering correction '{}'".format(full_path)
            )
...
class InterpolateGridDataCorrectionFromProvider(SymmetricEngineeringCorrection):
    ...
    def __init__(
        ...
        *beamline_parameters: List["BeamlineParameter"],
        ...
        self._beamline_parameters = [
            self._find_parameter(variable, beamline_parameters)
    ...
    def _find_parameter(
        ...
        if len(named_parameters) != 1:
            ...
            raise ValueError(
                "Data for Interpolate Grid Data has column name '{}' which does not match either "
                "'{}' or one of the beamline parameter '{}'".format(
                    parameter_name, COLUMN_NAME_FOR_DRIVER_SETPOINT, parameter_names
                )
            )
    ...
    def correction(self, setpoint: float) -> float:
        ...
        if None in evaluation_point:
            ...
            STATUS_MANAGER.update_error_log(
                "Engineering correction, '{}', evaluated for non-autosaved value, {}".format(
            ...
            interpolated_value = griddata(
                self._grid_data_provider.points,
