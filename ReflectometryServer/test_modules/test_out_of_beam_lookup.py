...
class TestComponentWithOutOfBeamPositions(unittest.TestCase):
    ...
    def test_GIVEN_multiple_out_of_beam_position_WHEN_getting_position_for_given_intercept_THEN_correct_position_returned(
        self, beam_height, expected
    ):
    ...
    def test_GIVEN_multiple_out_of_beam_positions_WHEN_checking_whether_component_is_in_beam_THEN_lookup_provides_correct_answer_for_given_beam_intersect(
        ...
        actual, _ = lookup.out_of_beam_status(
            beam_intercept, displacement, None, None
        )  # out of been distance and parking sequence index should not mater

        assert_that(actual, is_(expected))

    @parameterized.expand(
        [
            (PARK_LOW_POS, 0.5, False),
            (PARK_LOW_POS, 2, True),
            (PARK_LOW_POS, 4, True),
    ...
    def test_GIVEN_positions_with_tolerance_WHEN_checking_whether_component_is_in_beam_THEN_returns_correct_status_for_each_position(
        self, position_to_check, offset_from_pos, expected
    ):
        ...
        park_high_with_tolerance = OutOfBeamPosition(
            PARK_HIGH_POS, threshold=1, tolerance=tolerance_high
        )
        ...
        for position in [position_to_check + offset_from_pos, position_to_check - offset_from_pos]:
            in_beam_status, _ = lookup.out_of_beam_status(
                beam_intercept, position, None, None
            )  # only absolute position is used
    ...
    def test_GIVEN_positions_with_identical_thresholds_WHEN_creating_lookup_THEN_exception_thrown(
        self,
    ):
...
class TestComponentWithSequenceOutOfBeamPositions(unittest.TestCase):
    def test_GIVEN_sequence_number_is_None_WHEN_get_parking_position_THEN_last_position_in_sequence_returned(
        self,
    ):
        expected_value = 10
    ...
    def test_GIVEN_sequence_index_WHEN_get_parking_position_THEN_correct_position_in_sequence_returned_or_None(
        self, sequence, sequence_number, expected_value
    ):
        lookup = OutOfBeamLookup([OutOfBeamSequence(sequence)])
        beam_intercept = Position(0, 0)

        actual = lookup.get_position_for_intercept(beam_intercept).get_sequence_position(
            sequence_number
        )
    ...
    @parameterized.expand(
        [
            ([1, 2, 3, 4], 0, 1 + 0.1, True),  # within tolerance of sequence end
            (
                [1, 2, 3, 4],
                1,
                1 + 0.1,
                False,
            ),  # within tolerance of first sequence end but not current end
            ...
            (
                ...
                4,
                True,
            ),  # sequence index is none so it is at the end of the current sequence
            ([None, None, 3, 4], 1, 4, True),  # sequence position is None
            ([None, None, 3, 4], 4, 4, True),  # sequence index is greater than sequence length
        ]
    )
    def test_GIVEN_positions_with_tolerance_WHEN_checking_whether_component_is_at_end_of_sequence_THEN_returns_correct_status(
        self, sequence, parking_index, position, expected
    ):
        tolerance = 0.2
        lookup = OutOfBeamLookup([OutOfBeamSequence(sequence, tolerance=tolerance)])

        _, result = lookup.out_of_beam_status(
            Position(0, 0), position, None, parking_index=parking_index
        )  # only absolute position is used
        assert_that(result, is_(expected))

    def test_GIVEN_sequence_positions_ending_in_none_WHEN_creating_lookup_THEN_exception_thrown(
        self,
    ):
        positions = [1, None]
    ...
    def test_GIVEN_sequence_positions_with_none_between_values_WHEN_creating_lookup_THEN_exception_thrown(
        self,
    ):
        positions = [1, None, 2]
