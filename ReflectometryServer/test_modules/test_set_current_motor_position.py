...
class TestCurrentMotorPositionParametersToEven_inDriver(unittest.TestCase):
    ...
    def test_GIVEN_tracking_beamline_parameter_and_component_WHEN_set_position_to_THEN_component_has_set_position_on(
        self, beam_path_height
    ):
        ...
        component.beam_path_rbv.axis[ChangeAxis.POSITION].add_listener(
            DefineValueAsEvent, _listener
        )
    ...
    @parameterized.expand([(0,), (1,)])
    def test_GIVEN_angle_beamline_parameter_and_reflecting_component_WHEN_set_position_to_THEN_component_has_set_position_on(
        self, beam_path_angle
    ):
    ...
    @parameterized.expand([(0,), (1,)])
    def test_GIVEN_angle_beamline_parameter_and_tilting_component_WHEN_set_position_to_THEN_component_has_set_position_on(
        self, beam_path_angle
    ):
        incoming_beam = PositionAndAngle(0, 0, beam_path_angle)
    ...
    def test_GIVEN_angle_beamline_parameter_and_theta_component_WHEN_set_position_to_THEN_error(
        self,
    ):
    ...
    def test_GIVEN_beamline_parameter_and_DirectParameter_component_WHEN_set_position_to_THEN_component_has_set_position_on(
        self,
    ):
        def _listener(set_position_to):
            self.set_position_to = set_position_to
        ...
        parameter = create_parameter_with_initial_value(
            0, DirectParameter, "param", mock_jaws_wrapper
        )
    ...
    def test_GIVEN_tracking_beamline_parameter_and_component_WHEN_set_position_to_THEN_component_and_parameter_set_points_are_new_values_motor_is_not_moved(
        self,
    ):
        ...
        assert_that(
            component.beam_path_set_point.axis[ChangeAxis.POSITION].get_relative_to_beam(),
            is_(expected_position),
            "component setpoint",
        )
    ...
    def test_GIVEN_angle_beamline_parameter_and_component_WHEN_set_position_to_THEN_component_and_parameter_set_points_are_new_values(
        self,
    ):
        incoming_beam = PositionAndAngle(0, 0, 0)
        expected_position = 1
        ...
        assert_that(
            component.beam_path_set_point.axis[ChangeAxis.ANGLE].get_relative_to_beam(),
            is_(expected_position),
            "component setpoint",
        )
        assert_that(parameter.sp, is_(expected_position), "Parameter setpoint")
    ...
    def test_GIVEN_beamline_parameter_and_DirectParameter_component_WHEN_set_position_to_THEN_setpoint_is_set_to_new_position(
        self,
    ):
        expected_position = 1
        expected_mock_jaws_wrapper_value = 10
        mock_jaws_wrapper = create_mock_JawsCentrePVWrapper(
            "jaws_centre", expected_mock_jaws_wrapper_value, 1
        )
        ...
        assert_that(
            mock_jaws_wrapper.last_set_point_set,
            is_(expected_mock_jaws_wrapper_value),
            "sp should not be set",
        )
...
class TestCurrentMotorPositionEventsToMotor(unittest.TestCase):
    ...
    def test_GIVEN_displacement_driver_no_engineering_correction_WHEN_receive_set_angle_as_event_for_position_set_THEN_motor_position_is_not_set(
        self,
    ):
        expected_position = 1
