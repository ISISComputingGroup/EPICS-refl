...
class TestHeightDriver(unittest.TestCase):
    ...
    def test_GIVEN_backlash_distance_is_none_WHEN_backlash_distance_checked_THEN_returned_value_is_zero(
        self,
    ):
        expected = 0.0
    ...
    def test_GIVEN_backlash_velocity_is_none_WHEN_backlash_distance_checked_THEN_returned_value_is_zero(
        self,
    ):
        expected = 0.0
        self.height_axis.backlash_distance = None
    ...
    def test_GIVEN_component_with_height_setpoint_above_current_position_WHEN_calculating_move_duration_THEN_returned_duration_is_correct(
        ...
        expected = 2.0
        self.jaws.beam_path_set_point.axis[ChangeAxis.POSITION].set_relative_to_beam(
            target_position
        )
    ...
    def test_GIVEN_displacement_changed_WHEN_listeners_on_axis_triggered_THEN_listeners_on_driving_layer_triggered(
        self,
    ):
        listener = MagicMock()
        ...
        assert_that(
            self.jaws.beam_path_rbv.axis[ChangeAxis.POSITION].get_displacement(),
            is_(expected_value),
        )
...
class TestNonSynchronisedHeightDriver(unittest.TestCase):
    def setUp(self):
        ...
        self.jaws_driver = IocDriver(
            self.jaws, ChangeAxis.POSITION, self.height_axis, synchronised=False
        )

    def test_GIVEN_component_with_height_setpoint_below_current_position_and_not_synchronised_WHEN_calculating_move_duration_THEN_returned_0(
        self,
    ):
        target_position = -20.0
        expected = 0.0
    ...
    def test_GIVEN_move_duration_and_target_position_set_on_non_synchronised_axis_WHEN_moving_axis_THEN_computed_axis_velocity_is_correct_and_setpoint_set(
        self,
    ):
        target_position = 20.0
        target_duration = 4.0
...
class TestHeightAndTiltDriver(unittest.TestCase):
    def setUp(self):
        ...
        self.tilting_jaws_driver_disp = IocDriver(
            self.tilting_jaws, ChangeAxis.POSITION, self.height_axis
        )
        self.tilting_jaws_driver_ang = IocDriver(
            self.tilting_jaws, ChangeAxis.ANGLE, self.tilt_axis
        )
    ...
    def test_GIVEN_multiple_axes_need_to_move_WHEN_computing_move_duration_THEN_maximum_duration_is_returned(
        ...
        result = max(
            self.tilting_jaws_driver_disp.get_max_move_duration(),
    ...
    def test_GIVEN_move_duration_and_target_position_set_WHEN_moving_multiple_axes_THEN_computed_axis_velocity_is_correct_and_setpoint_set_for_all_axes(
        ...
        self.tilting_jaws.beam_path_set_point.set_incoming_beam(beam)
        self.tilting_jaws.beam_path_set_point.axis[ChangeAxis.POSITION].set_relative_to_beam(
            0.0
        )  # move component into beam
        ...
        assert_that(
            self.height_axis.velocity, is_(close_to(expected_velocity_height, FLOAT_TOLERANCE))
        )
...
class TestNonSynchronisedHeightAndTiltDriver(unittest.TestCase):
    def setUp(self):
        ...
        self.tilting_jaws_driver_ang = IocDriver(
            self.tilting_jaws, ChangeAxis.ANGLE, self.tilt_axis, synchronised=False
        )

    def test_GIVEN_multiple_axes_need_to_move_and_one_is_not_synchronised_WHEN_computing_move_duration_THEN_maximum_duration_is_returned(
        self,
    ):
        beam_angle = 45.0
        expected = 1.0
    ...
    def test_GIVEN_move_duration_and_target_position_set_and_one_is_not_synchronised_WHEN_moving_multiple_axes_THEN_computed_axis_velocity_is_correct_and_setpoint_set_for_all_axes(
        self,
    ):
        beam_angle = 45.0
        beam = PositionAndAngle(0.0, 0.0, beam_angle)
...
class TestHeightAndIocDriver(unittest.TestCase):
    def setUp(self):
        ...
        self.supermirror_driver_disp = IocDriver(
            self.supermirror, ChangeAxis.POSITION, self.height_axis
        )
    ...
    def test_GIVEN_multiple_axes_need_to_move_WHEN_computing_move_duration_THEN_maximum_duration_is_returned(
        ...
        target_angle = 30.0
        expected = 3.0
        self.supermirror.beam_path_set_point.axis[ChangeAxis.ANGLE].set_displacement(
        ...
        result = max(
            self.supermirror_driver_disp.get_max_move_duration(),
    ...
    def test_GIVEN_move_duration_and_target_position_set_WHEN_moving_multiple_axes_THEN_computed_axis_velocity_is_correct_and_setpoint_set_for_all_axes(
        ...
        self.supermirror.beam_path_set_point.axis[ChangeAxis.POSITION].set_relative_to_beam(
            10.0
        )  # move component into beam
    ...
    def test_GIVEN_angle_changed_WHEN_listeners_on_axis_triggered_THEN_listeners_on_driving_layer_triggered(
        ...
        listener.assert_called_once()
        assert_that(
            self.supermirror.beam_path_rbv.axis[ChangeAxis.ANGLE].get_displacement(),
            is_(expected_value),
        )
...
class TestHeightDriverInAndOutOfBeam(unittest.TestCase):
    ...
    def test_GIVEN_component_which_is_out_of_beam_WHEN_calculating_move_duration_THEN_returned_duration_is_0(
        self,
    ):
    ...
    def test_GIVEN_component_which_is_moving_into_beam_from_out_of_beam_WHEN_calculating_move_duration_THEN_returned_duration_is_0(
        self,
    ):
    ...
    def test_GIVEN_component_which_is_out_of_beam_WHEN_moving_axis_THEN_velocity_is_not_set_and_setpoint_is_set(
        self,
    ):
    ...
    def test_GIVEN_displacement_changed_to_out_of_beam_position_WHEN_listeners_on_axis_triggered_THEN_listeners_on_driving_layer_triggered_and_have_in_beam_is_false(
        self,
    ):
        listener = MagicMock()
        self.jaws.beam_path_rbv.add_listener(BeamPathUpdate, listener)
    ...
    def test_GIVEN_displacement_changed_to_an_in_beam_position_WHEN_listeners_on_axis_triggered_THEN_listeners_on_driving_layer_triggered_and_have_in_beam_is_true(
        ...
        self.height_axis.sp = (
            self.out_of_beam_position.get_final_position()
            + 2 * self.tolerance_on_out_of_beam_position
        )
    ...
    def test_GIVEN_displacement_changed_to_out_of_beam_position_within_tolerance_WHEN_listeners_on_axis_triggered_THEN_listeners_on_driving_layer_triggered_and_have_in_beam_is_false(
        ...
        self.height_axis.sp = (
            self.out_of_beam_position.get_final_position()
            + self.tolerance_on_out_of_beam_position * 0.9
        )
...
class TestDriverChanged(unittest.TestCase):
    ...
    def test_GIVEN_sp_value_set_and_moved_to_THEN_driver_reports_at_setpoint_and_changed_is_false(
        self,
    ):
        expected = True
        self.height_axis.sp = 0.0
    ...
    def test_GIVEN_sp_value_set_and_moved_to_but_no_distance_travelled_THEN_driver_reports_at_setpoint_and_changed_is_false(
        self,
    ):
        expected = True
        self.height_axis.sp = 0.0
    ...
    def test_GIVEN_component_sp_is_within_motor_resolution_WHEN_comparing_to_motor_setpoint_THEN_driver_reports_at_setpoint(
        self,
    ):
        expected = True
...
class BeamlineMoveDurationTest(unittest.TestCase):
    def setUp(self):
        ...
        supermirror = ReflectingComponent(
            "supermirror", setup=PositionAndAngle(y=0.0, z=10.0, angle=90.0)
        )
        sm_height_axis = create_mock_axis("SM:HEIGHT", 0.0, 10.0)
        sm_angle_axis = create_mock_axis("SM:ANGLE", sm_angle, 10.0)
        supermirror.beam_path_set_point.axis[ChangeAxis.ANGLE].set_displacement(
        ...
        self.drivers = [
            supermirror_driver_disp,
            supermirror_driver_ang,
            self.slit_2_driver,
            slit_3_driver,
        ...
        mode = BeamlineMode(
            "mode name",
            [self.smangle.name, slit_2_pos.name, slit_3_pos.name, det_pos.name, det_ang.name],
        )
    ...
    def test_GIVEN_driver_contains_disconnected_pv_WHEN_beamline_moves_THEN_status_set(self):
        ...
        self.slit_3_driver.perform_move = MagicMock(
            side_effect=UnableToConnectToPVException("A_PV", "ERROR")
        )
...
class BeamlineBacklashMoveDurationTest(unittest.TestCase):
    ...
    def test_GIVEN_two_axes_with_backlash_WHEN_triggering_move_THEN_components_move_at_speed_of_slowest_axis(
        ...
        detector = TiltingComponent(
            "point_detector", setup=PositionAndAngle(y=0.0, z=6.0, angle=90.0)
        )
        detector_height_axis = create_mock_axis(
            "HEIGHT", pos["start"], pos["max_vel"], pos["back_dist"], pos["back_speed"], pos["dir"]
        )
        detector_tilt_axis = create_mock_axis(
            "TILT", ang["start"], ang["max_vel"], ang["back_dist"], ang["back_speed"], ang["dir"]
        )
        detector_driver_disp = IocDriver(detector, ChangeAxis.POSITION, detector_height_axis)
        detector_driver_ang = IocDriver(detector, ChangeAxis.ANGLE, detector_tilt_axis)
        det_pos = AxisParameter("det_pos", detector, ChangeAxis.POSITION)
...
class TestIocDriverWithAxesDependentOnParam(unittest.TestCase):
    ...
    def test_GIVEN_component_WHEN_both_axis_moves_THEN_displacement_is_axis_value(self):
        ...
        assert_that(
            self.jaws.beam_path_rbv.axis[ChangeAxis.POSITION].get_displacement(), is_(expected)
        )

    def test_GIVEN_component_and_param_set_to_alt_WHEN_alt_axis_moves_THEN_displacement_is_alt_axis_value(
        self,
    ):
        expected = 1
    ...
    def test_GIVEN_component_and_axis_position_set_WHEN_change_param_THEN_displacement_changes(
        self,
    ):
        expected = 1
        self.height_axis.sp = 99
    ...
    def test_GIVEN_component_and_axis_position_set_WHEN_change_param_to_non_change_THEN_displacement_changes_to_default_axis(
        self,
    ):
        expected = 1
        self.height_axis.sp = expected
    ...
    def test_GIVEN_component_and_param_set_to_alt_WHEN_alt_axis_changes_THEN_change_is_alt_axis_value(
        self,
    ):
        expected = True
        self.param.sp = self.opt2
...
class TestIOCDriverInAndOutOfBeamWithParkingsequence(unittest.TestCase):
    def setUp(self):
        ...
        self.out_of_beam_position = OutOfBeamSequence(
            self.parking_sequence, tolerance=self.tolerance_on_out_of_beam_position
        )
    ...
    def test_GIVEN_parking_sequence_WHEN_ioc_driver_moves_to_be_at_end_of_sequence_THEN_axis_has_end_of_sequence_set(
        self,
    ):
        parking_index = 1
    ...
    def test_GIVEN_parking_sequence_WHEN_ioc_driver_moves_to_be_not_at_end_of_sequence_THEN_parking_sequence_not_changed(
        self,
    ):
