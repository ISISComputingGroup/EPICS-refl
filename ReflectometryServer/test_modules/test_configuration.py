...
PATH_TO_CONFGIS = os.path.abspath(os.path.join(os.path.dirname(__file__), "test_config", "refl"))
...
class TestConfiguration(unittest.TestCase):
    ...
    def test_WHEN_loading_valid_beamline_configuration_file_THEN_correct_PVs_are_created(self):
        beamline_configuration.REFL_CONFIG_PATH = PATH_TO_CONFGIS
    ...
    def test_WHEN_loading_beamline_configuration_file_that_doesnt_exist_THEN_status_PV_shows_import_error(
        ...
        beamline_configuration.REFL_CONFIG_PATH = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "refl")
        )
        with patch(
            "ReflectometryServer.beamline_configuration.import_module", side_effect=ImportError("")
        ) as import_fn:
            beamline = beamline_configuration.create_beamline_from_configuration({})
        #  Check Status PV
        self.assertEqual(
    ...
    def test_GIVEN_optional_macro_not_set_WHEN_loading_config_THEN_optional_items_not_part_of_config(
        self,
    ):
        beamline_configuration.REFL_CONFIG_PATH = PATH_TO_CONFGIS
        beamline = beamline_configuration.create_beamline_from_configuration(
