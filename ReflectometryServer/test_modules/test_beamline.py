...
TEST_PARAMETER_POSITION_SETS = [
    (
        ...
        (0, 0, 0),
        False,
    ),  # all axes start out of beam, moving in moves to 0
    (
        (-2, -3, -4),
        (0, -3, -4),
        (0, -3, -4),
        True,
    ),  # first axes in beam others out of beam, moving in moves to current positions
    (
        (-2, -3, -4),
        (-2, 0, -4),
        (-2, 0, -4),
        True,
    ),  # middle axes in beam others out of beam, moving in moves to current positions
    (
        (-2, -3, -4),
        (-2, -3, 0),
        (-2, -3, 0),
        True,
    ),  # last axes in beam others out of beam, moving in moves to current positions
    (
        (-2, -3, -4),
        (0, -3, -4),
        (0, -3, -4),
        True,
    ),  # first axes not in beam others in beam, moving in moves to current positions
    (
        (-2, -3, -4),
        (-2, 0, -4),
        (-2, 0, -4),
        True,
    ),  # middle axes not in beam others in beam, moving in moves to current positions
    (
        (-2, -3, -4),
        (-2, -3, 0),
        (-2, -3, 0),
        True,
    ),  # last axes not in beam others in beam, moving in moves to current positions
...
class TestComponentBeamline(unittest.TestCase):
    def setup_beamline(self, initial_mirror_angle, mirror_position, beam_start):
        ...
        mirror.beam_path_set_point.axis[ChangeAxis.ANGLE].set_displacement(
            CorrectedReadbackUpdate(initial_mirror_angle, None, None)
        )
    ...
    def test_GIVEN_beam_line_contains_multiple_component_WHEN_angle_on_mirror_changed_THEN_beam_positions_are_all_recalculated(
        ...
        mirror.beam_path_set_point.axis[ChangeAxis.ANGLE].set_displacement(
            CorrectedReadbackUpdate(mirror_final_angle, None, None)
        )
        results = [component.beam_path_set_point.get_outgoing_beam() for component in beamline]
...
class TestComponentBeamlineReadbacks(unittest.TestCase):
    def test_GIVEN_components_in_beamline_WHEN_readback_changed_THEN_components_after_changed_component_updatereadbacks(
        self,
    ):
        ...
        comp1.beam_path_rbv.axis[ChangeAxis.POSITION].set_displacement(
            CorrectedReadbackUpdate(1.0, None, None)
        )
...
class TestRealistic(unittest.TestCase):
    def test_GIVEN_beam_line_where_all_items_track_WHEN_set_theta_THEN_motors_move_to_be_on_the_beam(
        self,
    ):
        spacing = 2.0
    ...
    def test_GIVEN_beam_line_where_all_items_have_offset_WHEN_set_theta_THEN_motors_move_to_be_correct_distance_from_the_beam_theta_readback_is_correct_and_does_not_include_offset(
        self,
    ):
        spacing = 2.0
    ...
    def test_GIVEN_beam_line_where_all_items_track_WHEN_set_theta_no_move_and_move_beamline_THEN_motors_move_to_be_on_the_beam(
        self,
    ):
        spacing = 2.0
    ...
    def test_GIVEN_beam_line_where_all_items_track_but_one_axis_has_soft_limits_outside_of_range_WHEN_set_theta_no_move_and_move_beamline_THEN_motors_do_not_move(
        self,
    ):
        spacing = 2.0
        bl, drives = DataMother.beamline_s1_s3_theta_detector(spacing)
    ...
    def test_GIVEN_negative_sm_angle_WHEN_calculating_bench_intercept_THEN_value_accepted_and_no_error_thrown(
        self,
    ):
        expected_theta = 0.2
        ...
        bl, drives = DataMother.beamline_sm_theta_bench(
            0.0, expected_theta, driver_bench_offset, natural_angle=2.3
        )
...
class TestBeamlineValidation(unittest.TestCase):
    def test_GIVEN_two_beamline_parameters_with_same_name_WHEN_construct_THEN_error(self):
        ...
        assert_that(
            calling(Beamline).with_args([], [one, two], [], []),
    ...
    def test_GIVEN_enable_disable_parameter_with_driver_that_has_no_offset_WHEN_construct_THEN_error(
        ...
        assert_that(
            calling(Beamline).with_args([component], [beamline_parameter], [driver], [mode]),
            raises(BeamlineConfigurationInvalidException),
        )
    ...
    def test_GIVEN_beamline_with_LONG_AXIS_parameter_before_POSITION_WHEN_construct_THEN_no_error(
        self,
    ):
        mode = BeamlineMode("mode", [])
        component = TiltingComponent("comp", PositionAndAngle(0, 0, 90))
        beamline_parameters = [
            AxisParameter("param_1", component, ChangeAxis.LONG_AXIS),
    ...
    def test_GIVEN_beamline_with_POSITION_parameter_before_LONG_AXIS_WHEN_construct_THEN_error(
        self,
    ):
        mode = BeamlineMode("mode", [])
        component = TiltingComponent("comp", PositionAndAngle(0, 0, 90))
        beamline_parameters = [
            AxisParameter("param_1", component, ChangeAxis.POSITION),
            AxisParameter("param_2", component, ChangeAxis.LONG_AXIS),
        ]
        assert_that(
            calling(Beamline).with_args([component], beamline_parameters, [], [mode]),
            raises(BeamlineConfigurationInvalidException),
        )

    def test_GIVEN_beamline_with_LONG_AXIS_driver_before_POSITION_WHEN_construct_THEN_no_error(
        self,
    ):
        mode = BeamlineMode("mode", [])
        component = TiltingComponent("comp", PositionAndAngle(0, 0, 90))
        motor_axis = [create_mock_axis("axis_1", 0, 0), create_mock_axis("axis_2", 0, 0)]
        drivers = [
            IocDriver(component, ChangeAxis.LONG_AXIS, motor_axis[0]),
    ...
    def test_GIVEN_beamline_with_POSITION_driver_before_LONG_AXIS_WHEN_construct_THEN_error(self):
        ...
        motor_axis = [create_mock_axis("axis_1", 0, 0), create_mock_axis("axis_2", 0, 0)]
        drivers = [
            IocDriver(component, ChangeAxis.POSITION, motor_axis[0]),
            IocDriver(component, ChangeAxis.LONG_AXIS, motor_axis[1]),
        ]
        assert_that(
            calling(Beamline).with_args([component], [], drivers, [mode]),
            raises(BeamlineConfigurationInvalidException),
        )
...
class TestBeamlineModeInitialization(unittest.TestCase):
    ...
    def test_GIVEN_no_autosaved_mode_WHEN_instantiating_beamline_THEN_defaults_to_first_in_list(
        self, mode_autosave
    ):
    ...
    @patch("ReflectometryServer.beamline.mode_autosave")
    def test_GIVEN_autosaved_mode_exists_WHEN_instantiating_beamline_THEN_active_mode_is_saved_mode(
        self, mode_autosave
    ):
        expected = "pnr"
    ...
    @patch("ReflectometryServer.beamline.mode_autosave")
    def test_GIVEN_autosaved_mode_does_not_exist_in_config_WHEN_instantiating_beamline_THEN_mode_defaults_to_first_in_list(
        self, mode_autosave
    ):
        expected = "nr"
    ...
    @patch("ReflectometryServer.beamline.mode_autosave")
    def test_GIVEN_autosaved_mode_exists_WHEN_instantiating_beamline_THEN_mode_inits_are_not_applied(
        self, mode_autosave
    ):
    ...
    @patch("ReflectometryServer.beamline.mode_autosave")
    def test_GIVEN_default_mode_applied_WHEN_instantiating_beamline_THEN_mode_inits_are_not_applied(
        self, mode_autosave
    ):
    ...
    @patch("ReflectometryServer.beamline.mode_autosave")
    @patch("ReflectometryServer.beam_path_calc.disable_mode_autosave")
    def test_GIVEN_beam_line_with_disable_autosave_position_WHEN_init_THEN_incoming_beams_set_correctly_on_start(
        self, mock_disable_mode_auto_save, mock_mode_auto_save
    ):
        ...
        result = {
            comp.name: (
                comp.beam_path_set_point._incoming_beam,
                comp.beam_path_set_point.get_outgoing_beam(),
            )
            for comp in bl
        }
        ...
        assert_that(
            result[theta_comp_name][0], is_(position_and_angle(autosave_values[theta_comp_name]))
        )

        # The detector incoming beam should be the same as the outgoing beam for theta because theta controls the
        # detector height
        assert_that(
            result[detector_comp_name][0], is_(position_and_angle(result[theta_comp_name][1]))
        )


class TestRealisticWithAutosaveInit(unittest.TestCase):
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_theta_at_0_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        ...
        assert_that(
            bl.parameter("sm_angle").sp, is_(close_to(expected_sm_angle, 1e-6)), "sm angle SP"
        )
        ...
        assert_that(
            bl.parameter("sm_angle").sp_rbv,
            is_(close_to(expected_sm_angle, 1e-6)),
            "sm angle SP RBV",
        )
        assert_that(
            bl.parameter("theta").sp_rbv, is_(close_to(expected_theta, 1e-6)), "theta SP RBV"
        )
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_theta_at_non_zero_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 22.5
        expected_theta = 2
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_det_offset_at_zero_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 22.5
        expected_theta = 0
        ...
        bl, drives = DataMother.beamline_sm_theta_detector(
            expected_sm_angle, expected_theta, autosave_theta_not_offset=False
        )
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_det_offset_at_non_zero_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 22.5
        expected_theta = 0
        ...
        bl, drives = DataMother.beamline_sm_theta_detector(
            expected_sm_angle, expected_theta, expected_det_offset, autosave_theta_not_offset=False
        )
        ...
        assert_that(bl.parameter("theta").sp, is_(close_to(expected_theta, 1e-6)), "theta SP")
        assert_that(
            bl.parameter("det_pos").sp, is_(close_to(expected_det_offset, 1e-6)), "det position SP"
        )
        assert_that(bl.parameter("det_angle").sp, is_(close_to(0, 1e-6)), "det angle SP")
        ...
        assert_that(
            bl.parameter("det_pos").sp_rbv,
            is_(close_to(expected_det_offset, 1e-6)),
            "det position SP RBV",
        )
        assert_that(bl.parameter("det_angle").sp_rbv, is_(close_to(0, 1e-6)), "det angle SP RBV")

    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_with_nonzero_beam_start_with_all_values_at_0_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        ...
        bl, drives = DataMother.beamline_sm_theta_detector(
            expected_sm_angle, expected_theta, beam_angle=beam_start_angle
        )
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_with_nonzero_beam_start_where_autosave_theta_at_0_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        beam_start_angle = -2.3
        expected_sm_angle = 22.5
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_with_nonzero_beam_start_where_autosave_theta_at_non_zero_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        beam_start_angle = -2.3
        expected_sm_angle = 22.5
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_with_nonzero_beam_start_where_autosave_det_offset_at_zero_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        beam_start_angle = -2.3
        expected_sm_angle = 22.5
        ...
        bl, drives = DataMother.beamline_sm_theta_detector(
            expected_sm_angle,
            expected_theta,
            autosave_theta_not_offset=False,
            beam_angle=beam_start_angle,
        )
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_with_nonzero_beam_start_where_autosave_det_offset_at_non_zero_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        beam_start_angle = -2.3
        expected_sm_angle = 22.5
...
class TestRealisticWithAutosaveInitAndBench(unittest.TestCase):
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_theta_at_0p1_and_bench_offset_0_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 0
        expected_theta = 0.1
        ...
        bl, drives = DataMother.beamline_sm_theta_bench(
            expected_sm_angle, expected_theta, driver_bench_offset
        )
        ...
        assert_that(bl.parameter("theta").sp, is_(close_to(expected_theta, 1e-6)), "theta SP")
        assert_that(
            bl.parameter("bench_angle").sp,
            is_(close_to(driver_bench_offset, 1e-6)),
            "bench angle SP",
        )
        ...
        assert_that(
            bl.parameter("bench_angle").sp_rbv,
            is_(close_to(driver_bench_offset, 1e-6)),
            "bench angle SP RBV",
        )

        assert_that(
            bl.parameter("sm_angle").rbv, is_(close_to(expected_sm_angle, 1e-6)), "sm angle RBV"
        )
        assert_that(bl.parameter("theta").rbv, is_(close_to(expected_theta, 1e-6)), "theta RBV")
        assert_that(
            bl.parameter("bench_angle").rbv,
            is_(close_to(driver_bench_offset, 1e-6)),
            "bench angle RBV",
        )

    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_theta_at_0p1_and_bench_offset_2_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 0
        expected_theta = 0.1
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_bench_offset_2_and_theta_0p1_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 0
        expected_theta = 0.1
        ...
        bl, drives = DataMother.beamline_sm_theta_bench(
            expected_sm_angle, expected_theta, driver_bench_offset, autosave_bench_not_theta=True
        )
    ...
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_beam_line_where_autosave_ang_offset_2_and_theta_0p1_WHEN_init_THEN_beamline_is_at_given_place(
        self, file_io
    ):
        expected_sm_angle = 0
        expected_theta = 0.1
        ...
        bl, drives = DataMother.beamline_sm_theta_ang_det(
            expected_sm_angle, expected_theta, angle_offset, autosave_bench_not_theta=True
        )
        ...
        assert_that(bl.parameter("theta").sp, is_(close_to(expected_theta, 1e-6)), "theta SP")
        assert_that(
            bl.parameter("comp_angle").sp, is_(close_to(angle_offset, 1e-6)), "comp angle SP"
        )
        ...
        assert_that(
            bl.parameter("comp_angle").sp_rbv,
            is_(close_to(angle_offset, 1e-6)),
            "comp angle SP RBV",
        )
        ...
        assert_that(bl.parameter("theta").rbv, is_(close_to(expected_theta, 1e-6)), "theta RBV")
        assert_that(
            bl.parameter("comp_angle").rbv, is_(close_to(angle_offset, 1e-6)), "comp angle RBV"
        )
...
class TestBeamlineReadOnlyParameters(unittest.TestCase):
    ...
    def test_GIVEN_there_are_no_beamline_constant_set_empty_list_WHEN_get_beamline_constant_THEN_empty(
        self,
    ):
    ...
    def test_GIVEN_there_are_no_beamline_constant_set_using_none_WHEN_get_beamline_constant_THEN_empty(
        self,
    ):
    ...
    def test_GIVEN_there_are_beamline_constant_set_WHEN_get_beamline_constant_THEN_parameters_returned(
        ...
        expected_parameters = [BeamlineConstant("NAME", 2, "description")]
...
class TestComponentOutOfBeam(unittest.TestCase):
    ...
    def test_GIVEN_driver_on_component_has_no_out_of_beam_position_THEN_appropriate_change_axes_reports_not_having_out_of_beam_position(
        self, change_axis_to_set
    ):
        IocDriver(
            self.comp,
            change_axis_to_set,
            create_mock_axis("axis", 0, 1),
            out_of_beam_positions=None,
        )
    ...
    def test_GIVEN_driver_on_component_has_out_of_beam_position_THEN_appropriate_change_axis_report_having_out_of_beam_position(
        self, change_axis_to_set
    ):
    ...
    def test_GIVEN_component_with_driver_with_out_of_beam_position_WHEN_motor_is_in_beam_THEN_component_axis_reports_in_beam(
        self, change_axis_to_set
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        motor_axis = create_mock_axis("axis", self.IN_BEAM_VALUE, 1)
        IocDriver(
            self.comp, change_axis_to_set, motor_axis, out_of_beam_positions=[out_of_beam_position]
        )
        expected = True
    ...
    def test_GIVEN_component_with_driver_with_out_of_beam_position_WHEN_motor_is_out_of_beam_THEN_component_axis_reports_out_of_beam(
        self, change_axis_to_set
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
    ...
    def test_GIVEN_component_with_driver_with_out_of_beam_position_literal_WHEN_motor_is_out_of_beam_THEN_component_axis_reports_out_of_beam(
        self, change_axis_to_set
    ):
        motor_axis = create_mock_axis("axis", self.OUT_OF_BEAM_VALUE, 1)
        IocDriver(
            self.comp, change_axis_to_set, motor_axis, out_of_beam_positions=self.OUT_OF_BEAM_VALUE
        )
        expected = False
    ...
    def test_GIVEN_component_with_driver_with_out_of_beam_position_literal_WHEN_motor_is_in_beam_THEN_component_axis_reports_in_beam(
        self, change_axis_to_set
    ):
        motor_axis = create_mock_axis("axis", self.IN_BEAM_VALUE, 1)
        IocDriver(
    ...
    def test_GIVEN_component_with_one_driver_with_out_of_beam_position_WHEN_axis_is_not_in_beam_THEN_in_beam_rbv_is_false(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
    ...
    def test_GIVEN_component_with_multiple_drivers_with_out_of_beam_position_WHEN_all_are_in_beam_THEN_in_beam_rbv_is_true(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.IN_BEAM_VALUE, 1)
    ...
    def test_GIVEN_component_with_multiple_axes_with_out_of_beam_position_WHEN_none_are_in_beam_THEN_in_beam_rbv_is_false(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.OUT_OF_BEAM_VALUE, 1)
    ...
    def test_GIVEN_component_with_multiple_axes_with_out_of_beam_position_WHEN_one_is_in_beam_THEN_in_beam_rbv_is_true(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.OUT_OF_BEAM_VALUE, 1)
    ...
    def test_GIVEN_component_partially_in_beam_WHEN_setting_new_setpoint_for_axis_out_of_beam_THEN_driver_moves(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.IN_BEAM_VALUE, 1)
        psi_axis = create_mock_axis("psi", self.OUT_OF_BEAM_VALUE, 1)
        chi_driver = IocDriver(
            self.comp, ChangeAxis.CHI, chi_axis, out_of_beam_positions=[out_of_beam_position]
        )
        psi_driver = IocDriver(
            self.comp, ChangeAxis.PSI, psi_axis, out_of_beam_positions=[out_of_beam_position]
        )
        ...
        self.comp.beam_path_set_point.axis[ChangeAxis.PSI].set_displacement(
            CorrectedReadbackUpdate(self.IN_BEAM_VALUE, None, None)
        )
    ...
    def test_GIVEN_component_with_axis_with_out_of_beam_position_WHEN_in_beam_sp_changes_THEN_only_axis_with_out_of_beam_position_marked_as_changed(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.IN_BEAM_VALUE, 1)
    ...
    def test_GIVEN_component_WHEN_adding_driver_with_out_of_beam_position_THEN_inbeam_parameter_listens_to_relevant_axis(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.OUT_OF_BEAM_VALUE, 1)
    ...
    def test_GIVEN_inbeam_parameter_on_component_WHEN_adding_driver_with_out_of_beam_position_THEN_parameter_gets_init_value(
        self,
    ):
        out_of_beam_position = OutOfBeamPosition(self.OUT_OF_BEAM_VALUE)
        chi_axis = create_mock_axis("chi", self.OUT_OF_BEAM_VALUE, 1)
    ...
    def test_GIVEN_inbeam_parameter_on_detector_WHEN_init_with_angle_in_beam_THEN_theta_is_defined(
        self,
    ):
        out_of_beam_pos = -5
    ...
    def test_GIVEN_inbeam_parameter_on_detector_WHEN_init_with_pos_in_beam_THEN_theta_is_defined(
        self,
    ):
        out_of_beam_pos = -5
    ...
    def test_GIVEN_inbeam_parameter_on_detector_WHEN_init_with_both_out_of_beam_THEN_theta_sp_is_0(
        self,
    ):
        out_of_beam_pos = -5
        out_of_beam_pos_z = -2
        det_in, theta = DataMother.beamline_theta_detector(
            out_of_beam_pos_z, out_of_beam_pos_z, out_of_beam_pos, out_of_beam_pos
        )
...
class TestComponentOutOfBeamFullBeamline(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            (
                ...
                (-2, -3, -4),
                False,
            ),  # all axes start out of beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (0, -3, -4),
                (0, -3, -4),
                True,
            ),  # first axes in beam others out of beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (-2, 0, -4),
                (-2, 0, -4),
                True,
            ),  # middle axes in beam others out of beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (-2, -3, 0),
                (-2, -3, 0),
                True,
            ),  # last axes in beam others out of beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (0, -3, -4),
                (0, -3, -4),
                True,
            ),  # first axes not in beam others in beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (-2, 0, -4),
                (-2, 0, -4),
                True,
            ),  # middle axes not in beam others in beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (-2, -3, 0),
                (-2, -3, 0),
                True,
            ),  # last axes not in beam others in beam, moving in moves to autosaved positions
            (
                (-2, -3, -4),
                (1, 2, 3),
                (1, 2, 3),
                True,
            ),  # all axes in the beam, move to current position
        ]
    )
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_component_with_multiple_axes_out_of_beam_with_autosave_WHEN_init_and_move_in_THEN_component_moves_to_expected_palce(
        self,
        ...
        axis_ang, axis_phi, axis_pos, inbeam = self.setup_beamline(
            out_of_beam_positions, init_positions
        )
    ...
    def setup_beamline(
        self, out_of_beam_positions, init_positions, autosave_axes=True, autosave_inbeam=False
    ):
        ...
        param_pos = add_parameter(
            AxisParameter("pos", comp, ChangeAxis.POSITION, autosave=autosave_axes), [mode]
        )
        param_ang = add_parameter(
            AxisParameter("angle", comp, ChangeAxis.ANGLE, autosave=autosave_axes), [mode]
        )
        param_phi = add_parameter(
            AxisParameter("phi", comp, ChangeAxis.PHI, autosave=autosave_axes), [mode]
        )
    ...
    def test_GIVEN_component_with_multiple_axes_out_of_beam_no_auto_save_WHEN_init_and_move_in_THEN_component_moves_to_expected_palce(
        self, out_of_beam_positions, init_positions, expected_positions, expected_initial_inbeam
    ):
        axis_ang, axis_phi, axis_pos, inbeam = self.setup_beamline(
            out_of_beam_positions, init_positions, False
        )
    ...
    @parameterized.expand(TEST_PARAMETER_POSITION_SETS)
    @patch("ReflectometryServer.parameters.param_float_autosave")
    def test_GIVEN_component_with_multiple_axes_out_of_beam_with_autosave_not_no_value_WHEN_init_and_move_in_THEN_component_moves_to_expected_palce(
        self,
        out_of_beam_positions,
        init_positions,
        expected_positions,
        ...
        axis_ang, axis_phi, axis_pos, inbeam = self.setup_beamline(
            out_of_beam_positions, init_positions, True
        )
    ...
    def test_GIVEN_component_with_multiple_axes_out_of_beam_with_autosave_only_on_in_beam_WHEN_init_and_move_in_THEN_component_moves_to_expected_place_and_do_not_move_on_init(
        self, mock_autosave, mock_parking_index
    ):
        expected_position = 0
...
class TestComponentParkingSequence(unittest.TestCase):
    ...
    def test_GIVEN_component_with_one_axis_and_parking_sequence_WHEN_out_of_beam_THEN_component_moves_to_correct_places(
        self,
    ):
        pos1 = -3
        pos2 = -6
        ...
        driver = IocDriver(
            ...
            ChangeAxis.HEIGHT,
            mock_axis,
            out_of_beam_positions=[OutOfBeamSequence([pos1, pos2])],
        )
    ...
    def test_GIVEN_component_with_two_axes_and_parking_sequence_with_repeat_WHEN_out_of_beam_THEN_component_moves_to_correct_places(
        self,
    ):
        pos1 = -3
        ...
        driver1 = IocDriver(
            ...
            ChangeAxis.HEIGHT,
            mock_axis1,
            out_of_beam_positions=[OutOfBeamSequence([pos1, pos1, pos1])],
        )
        driver2 = IocDriver(
            comp,
            ChangeAxis.TRANS,
            mock_axis2,
            out_of_beam_positions=[OutOfBeamSequence(expected_sequence)],
        )
    ...
    def test_GIVEN_component_with_two_axes_and_parking_sequence_with_none_WHEN_out_of_beam_THEN_component_moves_to_correct_places(
        self,
    ):
        pos1 = -3
        expected_sequence = [4, 6, 8]
        ...
        driver1 = IocDriver(
            ...
            ChangeAxis.HEIGHT,
            mock_axis1,
            out_of_beam_positions=[OutOfBeamSequence([None, None, pos1])],
        )
        driver2 = IocDriver(
            comp,
            ChangeAxis.TRANS,
    ...
    @patch("ReflectometryServer.beam_path_calc.parking_index_autosave")
    def test_GIVEN_sequence_and_autosave_position_not_at_end_of_sequence_WHEN_init_THEN_init_error_and_autosave_overwriten(
        self, auto_save
    ):
        ...
        with self.assertRaises(BeamlineConfigurationParkAutosaveInvalidException):
            IocDriver(
                ...
                ChangeAxis.HEIGHT,
                mock_axis1,
                out_of_beam_positions=[OutOfBeamSequence([1, 2, 3])],
            )
    ...
    @parameterized.expand(
        [
            ...
            (
                5,
                [1, 2],
            ),  # after the last value in a sequence (it may be a different sequence this parked in)
        ]
    )
    @patch("ReflectometryServer.beam_path_calc.parking_index_autosave")
    def test_GIVEN_sequence_and_autosave_position_ok_WHEN_init_THEN_no_error(
        self, auto_save_value, sequence, auto_save
    ):
        ...
        IocDriver(
            comp, ChangeAxis.HEIGHT, mock_axis1, out_of_beam_positions=[OutOfBeamSequence(sequence)]
        )
    ...
    def test_GIVEN_component_with_two_axes_and_parking_sequence_WHEN_out_of_beam_and_then_in_beam_THEN_component_moves_to_correct_places(
        self,
    ):
        sequence1 = [4, 6, 8]
        ...
        driver1 = IocDriver(
            ...
            ChangeAxis.HEIGHT,
            mock_axis1,
            out_of_beam_positions=[OutOfBeamSequence(sequence1)],
        )
        driver2 = IocDriver(
            comp, ChangeAxis.TRANS, mock_axis2, out_of_beam_positions=[OutOfBeamSequence(sequence2)]
        )
        beamline = Beamline([comp], [parameter], [driver1, driver2], [BeamlineMode("mode", [])])
    ...
    def test_GIVEN_component_with_two_axes_and_parking_sequence_with_repeated_entry_at_end_WHEN_out_of_beam_and_then_in_beam_THEN_component_moves_to_correct_places(
        self,
    ):
        sequence1 = [4, 6, 6]
    ...
    def test_GIVEN_component_with_two_axes_and_parking_sequence_WHEN_motors_out_of_beam_and_init_THEN_motors_do_not_move(
        self,
    ):
        last_pos1 = 6
        ...
        comp.beam_path_set_point.in_beam_manager.add_rbv_in_beam_manager(
            comp.beam_path_rbv.in_beam_manager
        )
    ...
    def test_GIVEN_component_with_two_axes_and_parking_sequence_WHEN_motors_out_of_beam_and_init_with_autosaved_bean_out_THEN_motors_do_not_move(
        self,
    ):
        last_pos1 = 6
        sequence1 = [4, 1, last_pos1]
