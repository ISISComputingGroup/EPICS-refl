...
class TestComponentBeamline(unittest.TestCase):
    def setUp(self):
        ...
        self.ideal_sample_point = ReflectingComponent(
            "ideal sample point", setup=PositionAndAngle(0, 5, 90)
        )
        ...
        theta = create_parameter_with_initial_value(
            0, AxisParameter, "theta", self.ideal_sample_point, ChangeAxis.ANGLE
        )
        theta.sp_no_move = 0
        smangle = create_parameter_with_initial_value(
            0, AxisParameter, "smangle", self.polarising_mirror, ChangeAxis.ANGLE
        )
    ...
    def test_GIVEN_beam_line_contains_multiple_component_WHEN_set_theta_THEN_angle_between_incoming_and_outgoing_beam_is_correct(
        ...
        reflection_angle = (
            self.ideal_sample_point.beam_path_set_point.get_outgoing_beam().angle
            - self.ideal_sample_point.beam_path_set_point._incoming_beam.angle
        )
