...
class Component:
    ...
    def _init_beam_path_calcs(self, setup):
        ...
        self._beam_path_rbv = TrackingBeamPathCalc(
            "{}_rbv".format(self.name), LinearMovementCalc(setup)
        )
...
class TiltingComponent(Component):
    ...
    def _init_beam_path_calcs(self, setup):
        self._beam_path_set_point = SettableBeamPathCalcWithAngle(
            "{}_sp".format(self.name), LinearMovementCalc(setup), is_reflecting=False
        )
        self._beam_path_rbv = SettableBeamPathCalcWithAngle(
            "{}_rbv".format(self.name), LinearMovementCalc(setup), is_reflecting=False
        )
...
class ReflectingComponent(Component):
    ...
    def _init_beam_path_calcs(self, setup):
        self._beam_path_set_point = SettableBeamPathCalcWithAngle(
            "{}_sp".format(self.name), LinearMovementCalc(setup), is_reflecting=True
        )
        self._beam_path_rbv = SettableBeamPathCalcWithAngle(
            "{}_rbv".format(self.name), LinearMovementCalc(setup), is_reflecting=True
        )
...
class ThetaComponent(ReflectingComponent):
    ...
    def add_angle_to(self, component):
        ...
        self._beam_path_rbv.add_angle_to(
            component.beam_path_rbv, component.beam_path_set_point, axes
        )
    ...
    def add_angle_of(self, component):
        ...
        self._beam_path_rbv.add_angle_to(
            component.beam_path_rbv, component.beam_path_set_point, [ChangeAxis.ANGLE]
        )
    ...
    def _init_beam_path_calcs(self, setup):
        ...
        self._beam_path_rbv = BeamPathCalcThetaRBV(
            "{}_rbv".format(self.name), linear_movement_calc, self._beam_path_set_point
        )
...
class BenchComponent(TiltingComponent):
    ...
    def __init__(self, name: str, setup: BenchSetup):
        ...
        _, _, self._min_slide_position = self._calculate_motor_positions(
            0.0, self._min_angle_for_slide, 0.0
        )
        _, _, self._max_slide_position = self._calculate_motor_positions(
            0.0, self._max_angle_for_slide, 0.0
        )
        super(TiltingComponent, self).__init__(name, setup)
    ...
    def on_motor_axis_changed(self, update: AxisChangedUpdate):
        ...
        if not update.is_changed_update:
            ...
            any_have_unapplied_update = any(
                [set_point_axes[axis].is_changed for axis in self._motor_axes]
            )
    ...
    def _calculate_motor_rbvs(self, rbv_axis):
        ...
        if seesaw_sp == 0.0:
            ...
            height, pivot_angle, seesaw = self._calculate_pivot_height_and_angle_with_fixed_seesaw(
                front_jack, rear_jack, 0
            )
        else:
            ...
            height, pivot_angle, seesaw = (
                self._calculate_pivot_height_and_seesaw_with_fixed_pivot_angle(
                    front_jack, rear_jack, angle_sp
                )
            )
        return height, pivot_angle, seesaw

    def _calculate_pivot_height_and_seesaw_with_fixed_pivot_angle(
        self, front_jack, rear_jack, pivot_angle
    ):
        """
        ...
        height = (
            front_jack
            + rear_jack
            - (self._jack_front_z + self._jack_rear_z) * tan_bench_angle_sp
            + 2 * self._pivot_to_beam * one_minus_cos_angle_sp
        ) / 2.0
        seesaw = (
            (self._jack_rear_z - self._jack_front_z) * tan_bench_angle_sp + front_jack - rear_jack
        ) / 2.0
        return height, pivot_angle, seesaw
    ...
    def _calculate_pivot_height_and_angle_with_fixed_seesaw(self, front_jack, rear_jack, seesaw):
        ...
        front_jack -= seesaw
        rear_jack += seesaw
        tan_angle_from_initial_position = (front_jack - rear_jack) / (
            self._jack_front_z - self._jack_rear_z
        )
        angle_from_initial_position = atan(tan_angle_from_initial_position)
        height = (
            front_jack
            - self._jack_front_z * tan_angle_from_initial_position
    ...
    def on_set_relative_to_beam(self, _):
        ...
        front_jack_height, rear_jack_height, horizontal_position = self._calculate_motor_positions(
    ...
    def on_define_position(self, define_position: DefineValueAsEvent):
        ...
        if change_axis == self._position_axis:
            ...
            STATUS_MANAGER.update_error_log(
                "Define on bench using axis {} is not allowed".format(change_axis)
            )
