...
def create_beamline():
    ...
    polarising_mirror = ReflectingComponent(
        "Polarising mirror", setup=PositionAndAngle(0, 3, perp_to_floor)
    )
    ...
    ideal_sample_point = ReflectingComponent(
        "Ideal Sample Point", setup=PositionAndAngle(0, 5, perp_to_floor)
    )
...
def generate_theta_movement():
    ...
    positions_z = [
        component.beam_path_set_point.calculate_beam_interception().z for component in beamline
    ]
    ...
    for theta in range(0, 20, 1):
        ...
        positions_y = [
            component.beam_path_set_point.calculate_beam_interception().y for component in beamline
        ]
