...
class ProcessMonitorEvents():
    """
    Run through events generated by monitors regularly
    """

    monitor_events_lock = threading.RLock()
    monitor_events = {}

    def add_event(self, wrapper, what, listeners, new_value, alarm_severity, alarm_status):
        with ProcessMonitorEvents.monitor_events_lock:
            ProcessMonitorEvents.monitor_events[(wrapper, what)] = (listeners, new_value, alarm_severity, alarm_status)

    def __init__(self):
        proc_events = threading.Thread(target=self.process_events)
        proc_events.start()

    def process_events(self):
        """
        Process events on the events list

        """
        while True:
            try:
                with ProcessMonitorEvents.monitor_events_lock:
                    events_to_process = ProcessMonitorEvents.monitor_events
                    ProcessMonitorEvents.monitor_events = {}

                for listeners, new_value, alarm_severity, alarm_status in events_to_process.values():
...
class PVWrapper(object):
    ...
    def _add_monitors(self):
        ...
        """
        self._monitor_pv(self._rbv_pv, partial(self._trigger_listeners, "rbv", self._after_rbv_change_listeners))
        self._monitor_pv(self._sp_pv, partial(self._trigger_listeners, "sp", self._after_sp_change_listeners))
    ...
    def _trigger_listeners(self, what, listeners, new_value, alarm_severity, alarm_status):
        PROCESS_MONITOR_EVENTS.add_event(self, what, listeners, new_value, alarm_severity, alarm_status)
    ...
    def _on_update_moving_state(self, new_value, alarm_severity, alarm_status):
        ...
        self._trigger_listeners("changing", self._after_is_changing_change_listeners, self._dmov_to_bool(new_value),
